...
/**
  * LongTimeRangeWithShortTermPlanner knows about limited retention of raw data, and existence of downsampled data.
  ...
  * @param shortTermClusterPlanner this planner (typically a SingleClusterPlanner) abstracts planning for raw cluster data
  ...
  * @param earliestShortTermTimestampFn the function that will provide millis timestamp of earliest sample that
  *                             would be available in the short term cluster
  ...
  */
  class LongTimeRangeWithShortTermPlanner(rawClusterPlanner: QueryPlanner,
                             shortTermClusterPlanner: QueryPlanner,
                             downsampleClusterPlanner: QueryPlanner,
                             earliestRawTimestampFn: => Long,
                             earliestShortTermTimestampFn: => Long,
  ...
  private def materializeRoutablePlan(qContext: QueryContext, periodicSeriesPlan: PeriodicSeriesPlan): ExecPlan = {
    ...
    val earliestShortTime = earliestShortTermTimestampFn
    ...
    } else if (startWithOffsetMs - lookbackMs >= earliestShortTime) { // full time range in short term cluster
      shortTermClusterPlanner.materialize(periodicSeriesPlan, qContext)
